...
client_socket.connect(("141.223.140.44", 8088))
...
args = Namespace(classes='Noruway.names', config='Noruway.cfg',  weights='Noruway_400.weights')    # image='dog.jpg',
#args = Namespace(classes='yolov3.txt', config='yolov3.cfg',  weights='yolov2-tiny.weights')    # image='dog.jpg',
...
while True:
        ...
        for out in outs:
            for detection in out:
                ...
                if confidence > 0.5:
                    ...
                    print("YOLO_X:", type(x))
                    print("YOLO_Y:", type(y))
                    print("Yolo class:", len(class_ids))

                    # if we will use the first position, set like this.
                    if (len(class_ids) == 1) & init:
                        init_x = x
                        init_y = y
                        start_time = int(time.time())
                        init = False
                        #print("DO NOT!!!!!", init)
                        #print("*************************************X type:", init_x, type(init_x))
                    #time.sleep(0.5)
                    # if the object access the range of emergency,
                    # (suppose the range)
                    if (x <= 60) & end:
                        fin_x = x
                        fin_y = y
    ...
    if cv2.waitKey(1) & 0xFF == ord('q'):
        ...
        result, image_origin = cv2.imencode('.jpg', image, encode_param)
        #    data = zlib.compress(pickle.dumps(frame, 0))
        data = pickle.dumps(image_origin, 0)
        ...
        # y = 'LLSLSLSLSSLl'
        print()
        print("init_X:", int(init_x))
        print("init_Y:", int(init_y))


        print()
        print("fin_X:", int(fin_x))
        print("fin_Y:", int(fin_y))
        ...
        result_v = "%03s" % int((distance / (end_time - start_time)))
        #result_v = str(result_v)
        #print("PLEASEEEE", result_v)
        # -------------------------------------------------------------------------------------------------------
        # Following this line,
        #x_ = "77"
        #x_ = str(x_).encode()
        result_v = str(result_v).encode()
        print("result_v= ", result_v)

        client_socket.sendall(struct.pack(">L", size) + data + result_v)
